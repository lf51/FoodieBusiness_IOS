...
struct MonitorServizio: View {
       ...
       let(currentHour,currentDay,currentDate) = currentTime()
        ...
        case .adesso:
           ...
           //let allMenuToAnalize = self.viewModel.allMyMenu.filter({$0.isOnAir(checkTimeRange: true)})
            let allMenuToAnalize = self.viewModel.cloudData.allMyMenu.filter({$0.isOnAirValue().now})
        ...
        case .oggi:
            ...
            let allMenuToAnalize = self.viewModel.cloudData.allMyMenu.filter({$0.isOnAirValue().today})
        ...
        case .generale:
            ...
            let allMenuToAnalize = self.viewModel.cloudData.allMyMenu.filter({
    ...
    private func checkStatoServizio(rifIngOrPrep:[String]) -> StatoServizio {
        ...
        let mapStatoScorte = rifIngOrPrep.map({self.viewModel.cloudData.inventarioScorte.statoScorteIng(idIngredient: $0)})
                ...
                if showMore {
                    Grid(alignment:.leading) {
                        ...
                        vbSingleStatusRow(
                            ...
                            statusCheck: .disponibile,
                            path: \.cloudData.allMyIngredients)
...
struct MonitorModelView_Previews: PreviewProvider {
    static let allDish = testAccount.cloudData.allMyDish.map({$0.id})

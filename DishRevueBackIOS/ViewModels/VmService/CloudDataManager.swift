...
/*public final class GlobalDataManager {
  ...
  //  let propertiesManager:PropertyManager = PropertyManager()
   ...
   // let categoriesManager:CategoriesManager = CategoriesManager()
    ...
    deinit {
            print("[DEINIT]_GlobalDataManager")
        }
...
public final class CategoriesManager {
  ...
  /*  let joinedCategoriesPubliher = PassthroughSubject<[CategoriaMenu]?,Error>()
    ...
    func checkCategoryAlreadyExistInLibrary(categoria:CategoriaMenu,handle:@escaping(_ exist:Bool,_ documentID:String?) -> ()) {
        ...
        collectionManaged
            ...
            .getDocuments { query, error in
                ...
                guard let query,
                query.documents.isEmpty else {
                    // esiste
                    let first = query?.documents.first
                    let id = first?.documentID
                    handle(true,id)
                    return
                }
                handle(false,nil)
...
/*public final class PropertyManagerDEPRECATA {
...
/*
public final class CloudDataManagerDEPRECATA {
   // static var shared:CloudDataManager = CloudDataManager()
    private let db_base = Firestore.firestore()
